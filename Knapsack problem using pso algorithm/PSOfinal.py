...
def prof(x):
    ...
    for i in range(len(x)):
        total_prof += x[i] * val[i]  # the sum of profits for taken objects
    return total_prof
...
def w_kg(x, profit):
    total_kg = 0
    for i in range(len(x)):
        total_kg += x[i] * kg[i] #the sum of weights for taken objects

    if total_kg <= maxKg:
        return profit - 100*(min(0,(maxKg-total_kg))) #fitness equation  Q = 100
...
class Particle:
    def __init__(self,intial):
        ...
        self.fit_best_i=-1          # best fittness individual
        self.fit_i=-1               # fitness individual


        # intialization for velocity and position for particles
        for i in range(0,num_dimensions):
            self.velocity_i.append(random.uniform(0,1))
            self.position_i.append(intial[i])

    # evaluate current fitness for each particle
    def evaluate(self,costFunc):
        self.fit_i=costFunc(self.position_i)

        # check to see if the new position is an individual best
    ...
    def update_velocity(self,pos_best_g):
        ...
        for i in range(0,num_dimensions):
            ...
            # equation for updating veloctiy for each particle
...
class PSO:
    def __init__(self,initial,bounds,num_objects,particles,max_iter):
        ...
        for i in range(0,particles):
            swarm.append(Particle(initial)) # append each particle to the swarm (each solution)
